...
// auto-refresh (mutasi realtime) - simpan posisi scroll
(function(){
  var ms = window.__vgRefresh; if(!ms) return;
  
  window.addEventListener('beforeunload', function() {
    sessionStorage.setItem('vgScrollPos', window.scrollY);
  });
  document.addEventListener('DOMContentLoaded', function() {
    var pos = sessionStorage.getItem('vgScrollPos');
    if(pos) { window.scrollTo(0, parseInt(pos)); sessionStorage.removeItem('vgScrollPos'); }
  });
...
case 'generate':
 ...
 $cfg=provider_cfg(); $channel=$_POST['channel']??$cfg['default_channel'];
  ...
  // ANTI DOUBLE GENERATE: Cek apakah user punya TRX menunggu dlm 20 menit terakhir
  $cek_pending = pdo()->prepare("SELECT ref_no FROM transaksi WHERE username=? AND status='menunggu' AND merchant_id=? AND dibuat >= ? ORDER BY id DESC LIMIT 1");
  $cek_pending->execute([$username, mid(), date('Y-m-d H:i:s', strtotime('-20 minutes'))]);
  $pending = $cek_pending->fetchColumn();
  if($pending){
      flash('Anda masih memiliki tagihan yang belum dibayar. Selesaikan atau tunggu 20 menit!','err');
      redirect('qris',['ref'=>$pending]);
  }
...
case 'callback': 
  ...
  // LOCK PEMBUKUAN: Jika sudah sukses, abaikan request perubahan
  if($t['status'] === 'sukses'){
      echo "success"; break;
  }
...
case 'cek': require_login(); $ref=$_GET['ref']??''; $t=tx($ref); if($t && $t['status'] !== 'sukses'){ $prov=provider_status($t);
...
function view_qris($ref){ 
  ...
  if($t['status']==='sukses'){ 
    ...
    $h.='<h1 style="margin-top:0">Scan untuk Bayar</h1><div class="sub">Gunakan e-wallet / m-banking apa pun.</div><div class="timer">&#9201; Selesaikan pembayaran sebelum kedaluwarsa</div>';
    $h.='<div style="color:var(--red);font-weight:800;text-align:center;margin-bottom:10px;font-size:12px;background:var(--red-bg);padding:8px;border-radius:8px;">QRIS SEKALI PAKAI, KE MENU TOPUP LAGI UNTUK TOPUP SELANJUTNYA</div>';
    ...
    if($is_link) {
        ...
        $h.='<div style="width:100%; max-height: 50vh; aspect-ratio: 1/1; overflow:hidden; border-radius:12px; border:1px solid var(--line); margin-bottom:16px; background:#fff;">
...
function view_dashboard(){ $M=METODE();
  ...
  $expired_time = date('Y-m-d H:i:s', strtotime('-20 minutes'));
  ...
  if($es){ $w[]="dibuat >= ?"; $pr[]=$es." 00:00:00"; } 
  if($ee){ $w[]="dibuat <= ?"; $pr[]=$ee." 23:59:59"; }
  ...
  $b.='<a class="apitab '.($api=='all' && $status=='all' ?'on':'').'" href="'.u('dashboard',array_merge($common,['status'=>'all'])) .'"><span class="dot"></span>Semua</a>';
  // TAB KHUSUS SUKSES SAJA
  $b.='<a class="apitab '.($status=='sukses'?'on':'').'" style="border-color:var(--green); color:var(--green);" href="'.u('dashboard',array_merge($common,['status'=>'sukses'])).'"><span class="dot" style="background:var(--green)"></span>Hanya Sukses</a>';
