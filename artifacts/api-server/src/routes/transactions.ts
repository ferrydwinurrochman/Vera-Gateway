...
router.post("/generate", async (req, res) => {
  try {
    ...
    console.error("[transactions] POST /generate", error);
    res.status(500).json({ error: "Internal server error" });
  }
});
...
router.get("/sukses", async (req, res) => {
  try {
    ...
    console.error("[transactions] GET /sukses", error);
    res.status(500).json({ error: "Internal server error" });
  }
});
...
router.post("/callback", async (req, res) => {
  try {
    ...
    console.error("[transactions] POST /callback", error);
    res.status(500).json({ error: "Internal server error" });
  }
});
...
router.get("/:id", async (req, res) => {
  try {
    ...
    console.error("[transactions] GET /:id", error);
    res.status(500).json({ error: "Internal server error" });
  }
});
...
router.patch("/:id/status", async (req, res) => {
  try {
    ...
    console.error("[transactions] PATCH /:id/status", error);
    res.status(500).json({ error: "Internal server error" });
  }
});
  ...
  try {
    ...
    if (settings.flypayMode === "live" && settings.flypaySecret) {
      try {
        ...
        console.error("[transactions] POST /:ref/check — Flypay check failed", checkErr);
      }
    }
    ...
    console.error("[transactions] POST /:ref/check", error);
    res.status(500).json({ error: "Internal server error" });
  }
});
